...
const server = http.createServer((req, res) => {
  ...
  const tokenReq = https.request(options, (tokenRes) => {
    ...
    tokenRes.on('end', () => {
      ...
      writeFileSync(
        'f:\\tmp\\shopify.env',
        `SHOPIFY_ACCESS_TOKEN=${token}\nSHOPIFY_SHOP=${CONFIG.shop}\n`,
      );

      console.log('\n✅ Token obtenido y guardado en f:\\tmp\\shopify.env');
      console.log(`\nToken: ${token}\n`);

      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8' });
      res.end('<h1 style="font-family:sans-serif">✅ Autorizado. Podés cerrar esta ventana.</h1>');
...
server.listen(3000, () => {
  console.log('\n🔑  Shopify OAuth — Obteniendo access token');
  console.log('============================================');
  console.log('Abriendo el navegador...');
  console.log('\nSi no abre, copiá esta URL manualmente:\n');
  console.log(authUrl);
  exec(`start "" "${authUrl}"`);
});
